...
真实对局录入(只录公开信息,暗棋内容 AI 自动分配):

- **⬛ 填满暗棋**:一键全盘暗棋,再点掉空格、选中明棋设类型(中局录入最快)
- **📋 导入 / 📤 导出**:文本格式 `类型@行,列`(如 `R:J@0,0` 明棋、`H@2,3` 暗棋),一行一子,支持注释与轮到/分数还原;导出自动复制到剪贴板(公开视角,暗棋统一为 H)
- **已吃**:登记已被吃掉的棋子(如 `B:J,R:P`),暗棋随机分配时自动剔除,评估更贴近真实对局
- **🎲 随机开局**:生成完整 32 子随机局(暗棋显示 ■,可开「透视」查看内容);点击格子 → 右侧面板可手动编辑
